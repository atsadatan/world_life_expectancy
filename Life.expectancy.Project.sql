...
-- 50 Countries with the Highest life expectancy --

SELECT Country, `Life expectancy` ,status
FROM world_life_expectancy
...
-- 50 Countries with the Lowest life expectancy --
SELECT Country, `Life expectancy` ,status
FROM world_life_expectancy
where `Life expectancy` <> 0
...
select country , 
Round(AVG(`Life expectancy`),2) as life_exp, Round(AVG(BMI),1) as BMI,
status
from world_life_expectancy
group by country ,status
having life_exp <> 0 
